...
Yup.addMethod(Yup.string, 'phoneNumber', function(message) {
  return this.test('phoneNumberTest', message, function(value = '') {
    ...
    return (
      (!isError && (phoneNumber !== undefined && phoneNumber.isValid())) ||
...
export const ProfileSchema = Yup.object().shape({
  ...
  last_name: Yup.string()
    ...
    .min(1)
    .max(100)
    .required(i18n.t('errors.requirelast_name')),
  bio: Yup.string()
    .trim()
...
export const ContactSchema = Yup.object().shape({
  currentPasswordForEmail: Yup.string().min(8, i18n.t('errors.passwordMustBe')),
  email: Yup.string()
    .trim()
...
export const PasswordSchema = Yup.object().shape(
  {
    currentPassword: Yup.string().when(['newPassword', 'replyPassword'], {
    ...
    replyPassword: string().when(['currentPassword'], {
      ...
      then: string()
        .min(8, i18n.t('errors.passwordMustBe'))
        .max(256, i18n.t('errors.passwordMustBe'))
        .oneOf([Yup.ref('newPassword')], i18n.t('errors.confirmPassword'))
        .required(i18n.t('errors.passwordMustBe')),
      otherwise: string()
        .min(8, i18n.t('errors.passwordMustBe'))
        .max(256, i18n.t('errors.passwordMustBe')),
  ...
  [['currentPassword', 'newPassword'], ['currentPassword', 'replyPassword']],
);
...
const stripeCountriesList = countries.stripeCountriesList.map(i => i.title);
