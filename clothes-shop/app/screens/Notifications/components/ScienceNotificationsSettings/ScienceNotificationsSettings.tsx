...
const ScienceNotificationsSettings = ({
    addSubscription,
    removeSubscription,
    hasPermission,
    requestPermission,
    ...
    const [date, setDate] = React.useState(new Date().setHours(showHours))
             ...
             <ScrollView>
                <View style={{flexDirection:'row', flexWrap: 'wrap',flex:1, marginTop: 30, padding: 10}}>
                        ...
                        <CheckBox
                            checkedColor="red"
                            uncheckedColor="black"
                            title='Получать уведомления о новых тематические новостях, экспериментах и открытиях'
                            textStyle={globalStyles.text}
                            containerStyle={S.checkboxContainer}
                            // checkedIcon={<Image source={require('../checked.png')} />}
                            // uncheckedIcon={<Image source={require('../unchecked.png')} />}
                            checked={isSubscribed(Topics.science)}
                            onPress={() => toggleSubscription(Topics.science)}
                            />
                        {
                            /* <CheckBox
                                checkedColor="red"
...
export default ScienceNotificationsSettings;