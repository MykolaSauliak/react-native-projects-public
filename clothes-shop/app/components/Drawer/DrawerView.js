...
const Drawer = ({isAuthorized, user, items, goToAddNewItem, goToProfile}) => (
  <SafeAreaView style={s.container} forceInset={{bottom: 'never'}}>
    <View style={s.drawerContainer}>
      ...
      <View style={s.userContainer}>
        ...
        <View style={s.userProfileContainer}>
          {isAuthorized ? (
            <View>
              <Text bold black>
                {`${i18n.t('common.hello')}, ${user && user.profile.firstName}`}
              </Text>
              <TextTouchable
            ...
            <Text bold black>
              {`${i18n.t('common.hello')}, ${i18n.t('common.joinToUs')}`}
            </Text>
          )}
        </View>
      </View>
      <View style={s.drawerMain}>
        <View>
          {items.map(item => (
            <DrawerItem {...item} screen={item.screen} key={item.iconName} />
          ))}
        </View>
        {isAuthorized && (
