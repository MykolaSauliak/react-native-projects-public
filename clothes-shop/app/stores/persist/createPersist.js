...
const getSnapshots = (storesList, storage) => {
  const promises = storesList.map(storeName => storage.getItem(storeName));
...
const removeSnapshots = (storesList, storage) => {
  const promises = storesList.map(storeName => storage.removeItem(storeName));
...
const createPersist = (stores, config = {}) => {
  ...
  if (config.whitelist) {
    entries = entries.filter(([name]) => config.whitelist.includes(name));
  }
