...
export const Product = t
  .model('Product', {
    ...
    createdAt: t.maybe(t.maybeNull(t.Date)),
    ...
    metadata: t.maybeNull(t.model('metadata', {})),
    ...
    get nearestAvailableDate() {
      ...
      if (this.leaseStatus) {
        ...
        nearestAvailableDate =
          employedDates[0] || availableDates[availableDates.length - 1];
      }
...
function updateProduct(flow, store) {
  return function* updateProduct({images, ...params}) {
    try {
      ...
      const restImagesIds = images.filter(i => !i.type).map(i => String(i.id));
...
export const ListingsStore = t
  .model('ListingsStore', {
    ...
    fetchParticularUserListings: createFlow(fetchParticularUserListings),
  })
  .views(store => ({
    get Api() {
      return getEnv(store).Api;
...
function createListing(flow, store) {
  return function* createListing({
    ...
    try {
      ...
      const imagesId = resImages.map(item => item.data.data.id.uuid);
  ...
  return function* fetchListings({categories, title}) {
    try {
      ...
      const res = yield store.Api.fetchListings({
        ...
        pub_title: title,
        include: ['images', 'author', 'author.profileImage', 'reviews'],
      });
