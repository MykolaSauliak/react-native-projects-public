...
export const Transaction = t
  ...
  .views(store => ({
    ...
    get isViewer() {
      return store.relationships.listing.relationships.author.isViewer;
    },
...
function changeStateTransactions(flow, store) {
  return function* initiatechangeStateTransactionsTransaction({transition}) {
    try {
      flow.start();
...
function initiateOrderAfterEnquiry(flow, store) {
  return function* initiateTransaction({
    ...
    try {
      ...
      const cardToken = yield StripeService.createTokenWithCard(paramsToken);
...
export const TransactionStore = t
  .model('ListingsStore', {
    ...
    initiateMessageTransaction: createFlow(initiateMessageTransaction),
  ...
  .views(store => ({
    ...
    countAmount(bool) {
      return store.list.asArray
        .filter(
          i => i.isViewer === bool && i.lastTransition === 'transition/accept',
        )
        .reduce((acc, current) => {
...
function initiateMessageTransaction(flow, store) {
  return function* initiateMessage(listingId) {
    try {
      ...
      const res = yield flow.Api.initiateMessageTransaction(listingId);
...
function initiateTransaction(flow, store) {
  return function* initiateTransaction({
    ...
    try {
      ...
      const cardToken = yield StripeService.createTokenWithCard(params);
