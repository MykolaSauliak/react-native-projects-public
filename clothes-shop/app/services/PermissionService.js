...
class PermissionService {
  ...
  async _askForPermission(permission, options, onDenied, waitForSet) {
    try {
      ...
      let permissionResult = await Permissions.check(permission, options);
      ...
      if (permissionResult === 'undetermined') {
        permissionResult = await this._askPermissionNatively(permission);
  ...
  _showDeniedAlert(title, subtitle) {
    return () =>
      new Promise((res, rej) => {
        ...
        const showAlert = () =>
          AlertService.permissionDenied(title, subtitle, rej, onPress);
  ...
  _showIosRestrictedDialog(permission) {
    AlertService.iosRestrictedPermission(this.permissionsTitles[permission]);
